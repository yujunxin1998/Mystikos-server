...
| S3 排行榜与等级 | 陪玩榜/老板榜、贵宾等级升降级 | `mystikos-leaderboard` + `mystikos-membership` | PRD 当成一个服务，我们拆成两个限界上下文——**不是重复**，是更细的 DDD 边界，两者仍在同一个部署单元（`mystikos-app`）里，粒度不同不影响一起发布 |
| S4 亲密度与礼物成就 | 互动/赠礼记录、亲密度计算、成就解锁 | `mystikos-relationship` + `mystikos-gifting` | 同 S3，PRD 一个服务对应我们两个上下文，理由同上 |
| S5 商城与订单 | 商品、陪玩推荐关联、购物车、订单、库存 | `mystikos-commerce` | 吻合；`companion_product_endorsement`（陪玩需确认授权才能被关联推荐）是我们域模型目前没写清楚的细节，见第 3 节 |
...
1. **老板资料与隐私设置**：`nickname`、`privacy_anonymous`（是否匿名上榜）目前在 `mystikos-identity` 的 `User` 聚合里没有对应字段。归属建议：`mystikos-identity`（账号层面的基础设置，不属于 Membership 的消费驱动等级）。
