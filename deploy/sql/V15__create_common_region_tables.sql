-- 行政区划参考数据（国家 + 一级行政区两层树），mystikos-common-region 模块。
-- 主键直接用 ISO 3166-1 alpha-2（国家，如 'DE'）/ ISO 3166-2（一级行政区，如 'DE-BY'）编码，
-- 不用雪花算法代理ID——这是稳定的自然键，业务模块（如 identity_user.region_code）
-- 直接引用这个编码即可，不用先查一遍拿代理ID。
CREATE TABLE common_region (
    code        VARCHAR(10) PRIMARY KEY,
    parent_code VARCHAR(10) REFERENCES common_region (code),
...
CREATE INDEX idx_common_region_parent ON common_region (parent_code, sort_order);

COMMENT ON TABLE common_region IS '行政区划参考数据（国家 + 一级行政区），只读技术能力表，不属于任何业务限界上下文';
COMMENT ON COLUMN common_region.code IS 'ISO 3166-1 alpha-2（国家级）或 ISO 3166-2（一级行政区级）编码，主键';
COMMENT ON COLUMN common_region.parent_code IS '上级编码，国家级为空，一级行政区为所属国家的 code';
COMMENT ON COLUMN common_region.level IS '层级：COUNTRY / SUBDIVISION';
COMMENT ON COLUMN common_region.name_zh IS '中文展示名';
COMMENT ON COLUMN common_region.name_en IS '英文展示名';
COMMENT ON COLUMN common_region.sort_order IS '排序权重，越小越靠前';

-- 种子数据：UN M49 "Europe" 宏区域内 45 个主权国家 + 各自官方一级行政区（ISO 3166-2），
-- 共 1075 条一级行政区，逐条对照 Wikipedia 的 ISO 3166-2:XX 页面核实，不是凭记忆编的。
-- 不含从属领地/王室属地（法罗群岛、直布罗陀、奥兰群岛等）。
--
-- 几个不是笔误、故意如此的取舍，以后维护这张表之前先看这几条：
--   * 摩纳哥（MC）、梵蒂冈（VA）没有一级行政区（国情如此），只有国家行；
--   * 斯洛文尼亚（212 个）、马耳他（68 个）、北马其顿（80 个）三国的 ISO 3166-2 一级
--     直接编码到市镇一级（这三国本身没有"省/州"这层建制），种子数据量因此比其他国家
--     大得多，这是真实数据形态，不是数据错误；
--   * 科索沃（XK）：ISO 3166-1 本身不承认科索沃，也没有官方 ISO 3166-2 子编码——
--     `XK-FE`/`XK-GJK` 等 7 个区编码是为了内部一致性构造的，不是任何标准的一部分；
--   * 塞尔维亚（RS）故意不含 `RS-KM`（科索沃-梅托希亚自治省）及其下属5个区——
--     因为科索沃已经作为独立国家行存在，两边都占同一块地皮会在树里造成混乱重叠；
--   * 法国（FR）只列 18 个真正的大区（régions，含科西嘉），不含巴黎（FR-75C）和
--     里昂大都市（FR-69M）——这两个虽然有省级 ISO 编码，但属性是"特殊地位省级集合体"，
--     不是大区；
--   * 荷兰（NL）不含 3 个加勒比海特殊市镇（博纳尔/萨巴/圣尤斯特歇斯）——虽然它们有
--     荷兰的 ISO 编码，但不属于欧洲地理范围；
--   * 希腊（GR）不含阿索斯山（GR-69）——自治修道院实体，不是跟其他大区平级的行政区；
--   * 爱尔兰（IE）用 4 个省（provinces）而不是郡（counties）——ISO 3166-2 定义省是第一级，
--     郡是更深一级，且郡今天已经没有实际行政职能；
--   * 挪威（NO）用当前 15 个郡（2024 年 Viken 等合并郡拆分回原状之后的结构），
--     不是部分资料仍在用的旧 11 郡结构。
--
-- 生僻市镇（尤其斯洛文尼亚/北马其顿/马耳他/黑山/拉脱维亚那几百个）的中文名没有
-- 标准译名，用的是音译，仅供参考展示，真要用在用户可见 UI 上建议先抽查校对。
-- Country level
